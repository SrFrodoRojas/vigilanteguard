...
class ScanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($lastScan) {
            ...
            $reasons = [];
            if ($speed > $speedMaxMps) $reasons[] = "speed>{$speedMaxMps}m/s";
            if ($dt < $jumpWindowS && $dd > $jumpMaxM) $reasons[] = "jump>{$jumpMaxM}m<{$jumpWindowS}s";
            if ($reasons) {
                $suspect = 1;
                $suspectReason = implode('|', $reasons);
                // $verified = 0; // si querés forzar no verificado
            }
        }
        ...
        // Guardar con tipos de tu schema (SMALLINT UNSIGNED)
        ...
        // Redirigir SIEMPRE al GET con c & a para que la vista bloquee el botón
        return $verified
