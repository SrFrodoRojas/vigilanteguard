...
@section('content')
    ...
    @php
        if (Route::has('admin.patrol.routes.checkpoints.index')) {
            $checkpointsUrl = route('admin.patrol.routes.checkpoints.index', $route);
        } elseif (Route::has('admin.patrol.checkpoints.index')) {
            $checkpointsUrl = route('admin.patrol.checkpoints.index', ['route' => $route->id]);
        } else {
            $checkpointsUrl = url('/admin/patrol/routes/' . $route->id . '/checkpoints');
        }
    @endphp

    <div class="mb-3 d-flex flex-wrap gap-2">
        <a href="{{ $checkpointsUrl }}" class="btn btn-outline-secondary btn-sm">
            <i class="fas fa-map-marker-alt"></i> Administrar checkpoints
        </a>
        <a href="{{ route('admin.patrol.routes.index') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <form method="POST" action="{{ route('admin.patrol.routes.update', $route) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="expected_duration_min" min="1" step="1" class="form-control"
                        value="{{ old('expected_duration_min', $route->expected_duration_min) }}" required>
                </div>

                <div class="col-md-4">
                    <label class="form-label">Radio mínimo (m) *</label>
                    <input type="number" name="min_radius_m" min="1" step="1" class="form-control"
                        value="{{ old('min_radius_m', $route->min_radius_m) }}" required>
                </div>
